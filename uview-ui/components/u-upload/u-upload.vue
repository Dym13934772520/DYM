 ...
 * @property {String Number} max-count 最大Picture的数量（默认99）
 * @property {Boolean} custom-btn 如果需要自定义Picture的按钮，设置为true（默认false）
 ...
 * @property {Array<String>} source-type Picture的来源，album-从相册选图，camera-使用相机，默认二者都有（默认['album', 'camera']）
 * @property {Boolean} preview-full-image	是否可以通过uni.previewImage预览已选择的图片（默认true）
 * @property {Boolean} multiple	是否开启图片多选，部分安卓机型不支持（默认true）
 * @property {Boolean} deletable 是否显示删除图片的按钮（默认true）
 * @property {String Number} max-size 选择单个文件的最大大小，单位B(byte)，默认不限制（默认Number.MAX_VALUE）
 * @property {Array<Object>} file-list 默认显示的图片列表，数组元素为对象，必须提供url属性
 * @property {Boolean} upload-text Picture按钮的Tip文字（默认“Picture”）
 * @property {Boolean} auto-upload 选择完图片是否自动上传，见上方说明（默认true）
 * @property {Boolean} show-tips 特殊情况下是否自动Tiptoast，见上方说明（默认true）
 ...
 * @event {Function} on-choose-complete 每次Picture后触发，只是让外部可以得知每次选择后，内部的文件列表
...
export default {
	...
	props: {
		...
		// 内部预览图片区域和Picture按钮的区域宽度
		width: {
			type: [String, Number],
			default: 200
		},
		// 内部预览图片区域和Picture按钮的区域高度
		height: {
			type: [String, Number],
			default: 200
		},
	...
	methods: {
		...
		selectFile() {
			...
			// 设置为只Picture的时候使用 chooseImage 来实现
