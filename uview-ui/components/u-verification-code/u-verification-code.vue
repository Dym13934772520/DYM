...
<script>
	 ...
	 * @description 考虑到用户实际发送验证码的场景，可能是一个按钮，也可能是一段文字，Tip语各有不同，所以本组件 不提供界面显示，只提供Tip语，由用户将Tip语嵌入到具体的场景
	 * @tutorial https://www.uviewui.com/components/verificationCode.html
	 * @property {Number String} seconds 倒计时所需的秒数（默认60）
	 * @property {String} start-text 开始前的Tip语，见官网说明（默认获取验证码）
	 * @property {String} change-text 倒计时期间的Tip语，必须带有字母"x"，见官网说明（默认X秒重新获取）
	 * @property {String} end-text 倒计结束的Tip语，见官网说明（默认重新获取）
	...
	export default {
		...
		methods: {
			...
			start() {
				...
				// 这里放这句，是为了一开始时就Tip，否则要等setInterval的1秒后才会有Tip
				this.changeEvent(this.changeText.replace(/x|X/, this.secNum));
				this.setTimeToStorage();
				this.timer = setInterval(() => {
					if (--this.secNum) {
						// 用当前倒计时的秒数替换Tip字符串中的"x"字母
						this.changeEvent(this.changeText.replace(/x|X/, this.secNum));
					} else {
						clearInterval(this.timer);
						this.timer = null;
